...
const send_email = async (req, res) => {
  ...
  const send_email_confirm = async (req, res) => {
    ...
    for (var i=0; i < products.length; i++) {
      productsHTML = productsHTML + `<p>Name: ${products[i].name} Price:${products[i].price} Quantity:   ${products[i].quantity} Photo: <img src=${products[i].photo} alt="productImg" width="500" height="500"></img></p>`
    }
